...
export default function CustomerPortalPage() {
  ...
  const [orderQuery, setOrderQuery] = useState<string>('ORD-2026-1001');
  const [activeOrderNumber, setActiveOrderNumber] = useState<string>('ORD-2026-1001');
  const [selectedItemIds, setSelectedItemIds] = useState<string[]>([]);
  const [reason, setReason] = useState<string>('The dress arrived with a torn seam and damaged zipper.');
  ...
  // 1. Fetch Order by ID or Order Number
  ...
  // When order changes, pre-select the first item
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-16">
      ...
      <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200">
        <form onSubmit={handleSearchOrder} className="flex gap-3">
          ...
          <button
            ...
            className="px-5 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white text-sm font-semibold transition-all disabled:opacity-50"
          >
          ...
          <form onSubmit={handleSubmitRefund} className="pt-3 border-t border-slate-100 space-y-3">
            ...
            <div className="flex items-center justify-between pt-1">
              <span className="text-xs text-slate-400">
                Processed according to store return policy rules.
              </span>
              <button
                type="submit"
                disabled={evaluateMutation.isPending || selectedItemIds.length === 0 || !reason.trim()}
                className="flex items-center gap-2 px-6 py-2.5 rounded-xl bg-sky-600 hover:bg-sky-700 text-white text-sm font-semibold shadow-sm transition-all disabled:opacity-50"
              >
      ...
      {/* Step 4: Visible AI-Powered Decision Response */}
      {decision && (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-slate-200 animate-in fade-in slide-in-from-bottom-2 duration-200 space-y-4">
          ...
          {/* Prompt Injection / Security Notice */}
          {decision.promptInjectionDetected && (
            <div className="p-3.5 rounded-xl bg-rose-50 border border-rose-200 text-xs text-rose-900 flex items-start gap-2.5">
              ...
              <div>
                <span className="font-bold text-rose-800 block">Security Heuristic Intercepted</span>
                <p className="text-rose-700 text-[11px] mt-0.5">
                  Input contained policy override directives. The system safely quarantined the statement and routed the ticket for supervisory review.
                </p>
              </div>
            </div>
          )}
          ...
          <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
            <h4 className="text-xs font-bold uppercase tracking-wider text-slate-500 mb-1.5 flex items-center gap-1.5">
              <Headphones className="w-3.5 h-3.5 text-sky-600" />
              <span>Support Specialist Message</span>
            </h4>
          ...
          {decision.policyCitations.length > 0 && (
            <div className="text-xs">
              <span className="font-semibold text-slate-600 block mb-1">Applicable Store Policy:</span>
          ...
          <div className="pt-3 border-t border-slate-100 flex items-center justify-between text-xs">
            ...
            >
              <span>View Audit Record in Support Dashboard</span>
